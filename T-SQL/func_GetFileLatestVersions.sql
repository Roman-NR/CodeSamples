CREATE FUNCTION [dbo].[func_GetFileLatestVersions]
(
	@fileGuids [dbo].[GuidList] READONLY,
	@workspaceId int,
	@mapped int
)
RETURNS 
...
(
	...
	WHERE (@mapped = 0 OR (ISNULL(af.[IsCheckedOut], 0) = 0 AND (ISNULL(wf.[FileEntityPK], 0) <> ISNULL(af.[PK], 0) OR wf.[NeedUpdate] = 1)))
	  AND (af.[Guid] IN (SELECT [Value] FROM @fileGuids) OR wf.[FileEntityGuid] IN (SELECT [Value] FROM @fileGuids))
)
GO